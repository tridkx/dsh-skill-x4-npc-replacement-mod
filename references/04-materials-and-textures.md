...
### §7.4.1 **占位贴图不要用 16×16 纯色**

没有 albedo 的材质（缝线/底座一类）需要一张占位图。**实测 16×16 的纯色 DDS 会被
游戏拒绝，回退成"贴图缺失"的洋红** —— 于是本来想修的可疑色块反而变成了刺眼的洋红片。

**做法：直接复用主材质那张已经验证过的 1024px 贴图**（把 `diffuse_map` 指过去）。
颜色自然一致，代价只是平铺 UV 会让纹理重复 —— 而那些部件本来面积就小。
